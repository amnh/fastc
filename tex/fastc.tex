...
can lead to less than easily human-legible data files such as:\\
...
representation of alleleic polymorphism and assembly ambiguity.  FASTG and FASTC do not appear to be suffficiently compatible to merge the specifications at present.

\section{FASTC}
The FASTC format grows out of the FASTA and custom alphabet formats by adding a mandatory white space between elements and allowing for non-prefix free multi-character sequence element specification. A description of the file format is as follows:

A sequence specification in FASTC format begins with a single-line identifier, followed by one or more lines of sequence data. Many identifier lines and accompanying lines of sequence data maybe included in a FASTC file. A FASTC file must have at least one sequence specified, empty files are not valid.
...
The identifier of the sequence cannot include a whitespace or any of the following characters: `;', `>', `[', `]'.
An identifier may end early on the identifier line when a `;' symbol is encountered. Everything after a `;' symbol on the identifier line is considered a comment.

A line of sequence data contains one or more elements of the sequence seperated by spaces.
A sequence element is either an unambiguous state represented by a lexeme, or an ambiguous state of many lexemes seperated by whitespace and enclosed by `[' and `]' symbols. A lexeme is one or more contiguous characters. A lexeme cannot include whitespace or any of the following characters: `;', `>', `[', `]'.
...
Multiple identifier lines without sequence data  are not permitted. This would imply that there are missing data for these terminals. However there are alternative, higher-level techniques for infering missing data and it is outside the scope of this file format. This behavior has been inherited from the FASTA format.

The above description provides an inuitive understanding of the FASTC file format and it's usage. However, to use the FASTC file format in any software packages, a file parser that accepts the FASTC file format will need to be written. To aid future file parser implementors, we have provided a Backus normal form (BNF) \cite{Naur1961} context-free grammar specifcation of the FASTC file format below.
